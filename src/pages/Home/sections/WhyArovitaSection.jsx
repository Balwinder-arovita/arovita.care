...
export default function WhyArovitaSection() {
  ...
  return (
    <section className="testimonials" id="why-arovita">
      <div className="container">
        <div className="fu" style={{textAlign:'center',marginBottom:'60px'}}>
          <div className="sec-label">Why Arovita</div>
          <h2 className="sec-title">Healthcare today is <em>fragmented.</em><br/>Care should be connected.</h2>
          <p style={{fontSize:'15px',color:'var(--muted)',maxWidth:'520px',margin:'18px auto 0',lineHeight:'1.76'}}>Patients move between consultations, tests, and treatments, but these steps rarely connect. Arovita brings everything into one unified healthcare journey.</p>
        </div>

        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'24px',marginBottom:'24px'}}>
          {/* Problem */}
          <div className="fu d1" style={{background:'linear-gradient(145deg,#FFF5F5,#FFE8E8)',border:'1.5px solid rgba(220,80,80,.15)',borderRadius:'28px',padding:'36px',boxShadow:'0 4px 28px rgba(220,80,80,.06)'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:0}}>
              {problems.map((p,i) => (
                <div key={i} style={{display:'flex',alignItems:'center',gap:'14px',padding:'13px 0',borderBottom:i < problems.length-1 ? '1px solid rgba(220,80,80,.1)' : 'none'}}>
                  ...
                  <span style={{fontSize:'14px',color:'var(--ink2)',lineHeight:'1.5'}}>{p}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Solution */}
          <div className="fu d2" style={{background:'linear-gradient(145deg,#E6F7F0,#C8EFE0)',border:'1.5px solid rgba(26,122,90,.18)',borderRadius:'28px',padding:'36px',boxShadow:'0 4px 28px rgba(26,122,90,.08)',position:'relative',overflow:'hidden'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:0}}>
              {solutions.map((s,i) => (
                <div key={i} style={{display:'flex',alignItems:'center',gap:'14px',padding:'13px 0',borderBottom:i < solutions.length-1 ? '1px solid rgba(26,122,90,.1)' : 'none'}}>
                  ...
                  <span style={{fontSize:'14px',color:'var(--ink2)',lineHeight:'1.5'}}>{s}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Bridge statement */}
        <div className="fu d3" style={{background:'linear-gradient(135deg,var(--dark2) 0%,#1a3a2a 100%)',borderRadius:'24px',padding:'32px 40px',display:'flex',alignItems:'center',gap:'28px',boxShadow:'0 18px 56px rgba(9,24,16,.13)'}}>
          <div style={{width:'52px',height:'52px',background:'linear-gradient(135deg,var(--g),var(--gl))',borderRadius:'16px',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,boxShadow:'0 8px 24px rgba(46,196,138,.35)'}}>
          ...
          <div>
            ...
            <div style={{fontSize:'16px',color:'rgba(255,255,255,.82)',fontWeight:400,lineHeight:'1.7'}}>Arovita transforms isolated healthcare interactions into a continuous, connected care journey — where consultations lead to tests, tests lead to treatment, and treatment leads to ongoing monitoring.</div>
          </div>
        </div>
      </div>
    </section>
